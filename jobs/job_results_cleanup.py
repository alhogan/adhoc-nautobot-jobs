...
class ClearJobResultsJob(Job):
    ...
    def run(self, data, commit):
        ...
        jobs_to_delete = JobResult.objects.filter(
            completed__lte=str(
                make_aware(datetime.datetime.now()) - datetime.timedelta(days=end_time)
            )
        )
        for job_result in jobs_to_delete:
            if not commit:
                self.log_info(
                    obj=job_result,
                    message=f"{job_result} would be removed if Commit changes was selected.",
                )
        if commit:
            jobs_to_delete.delete()
            self.log_success(
                obj=None,
                message=f"Job results older than {end_time} days old have been deleted.",
            )
